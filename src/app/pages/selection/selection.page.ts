...
export class SelectionPage implements OnInit, OnDestroy {
  ...
  async loadTable(event?: Event) {
    ...
    this.mainDashboardSub = this.assetService
      .getTestNoForAssets(this.page)
      .subscribe((data) => {
        this.nextpg = data.next ? data.next.page : null;
  ...
  async setAssetAsFunctional(aid: string) {
    ...
    if (!this.desktop) {
      try {
        ...
        let ret = db.run(
          `UPDATE asset SET Status = "Functions" WHERE id = ? `,
          [aid]
        );
      ...
      this.setAssetAsFunctionalSub = this.assetService
        .setAssetAsFunctional(aid)
        .subscribe(async (data) => {
          ...
          if ((data = "Asset Set as Fixed")) {
            ...
            this.showAlert({
              head: "Failed",
              msg: "Could not set asset as fixed",
            });
          }
        });
    }
